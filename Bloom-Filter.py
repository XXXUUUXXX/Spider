...
代码中使用了MD5加密压缩，将字符串压缩到32个字符(也可用hashlib.sha1()压缩成40个字符)
作用：
    1.Bloom-Filter对一个很长的字符串哈希映射时会出错，经常误判为已存在，压缩后不会出现这个问题
    2.压缩后字符为0~f共16种可能，截取前两个字符，再跟进blockNum将字符串指定到不同的去重块进行去重
在使用过程中，适当地调整seed的数量和blockNum数量(seed越少去重速度越快，但漏失率越大)
--------------------------------------------------------------------------------
